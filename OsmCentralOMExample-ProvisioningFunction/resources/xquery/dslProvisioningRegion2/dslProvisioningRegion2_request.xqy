...
let $params := $order/oms:_root/oms:ResponseParamGroup/oms:ResponseParam
...
log:info($log,fn:concat('creating oms request[',/oms:GetOrder.Response/oms:OrderID,'] count[',count(/oms:GetOrder.Response/oms:_root/oms:data),']')),
<activationOrderAdsl xmlns="http://xmlns.oracle.com/communications/sce/dictionary/OsmCentralOMExample/dsl_region2"
 ...
 xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
     <order>
        ...
        {
        if (exists($params[oms:paramName/text() = 'FIXED_CENTRAL']))
        then (
          <centralId>{$params[oms:paramName/text() = 'FIXED_CENTRAL']/oms:paramValue/text()}</centralId>
        ...
        {
        if (exists($params[oms:paramName/text() = 'FIXED_PORT']))
        then (
          <numPort>{$params[oms:paramName/text() = 'FIXED_PORT']/oms:paramValue/text()}</numPort>
        ...
        <idProductADSL>323421O</idProductADSL>
        <customerAccount>
